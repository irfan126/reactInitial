...
export class BwmFileUpload extends React.Component {
  ...
  resetToDefaultState(status, displayUploadedImage) {
    this.setState({
      pending: false,
      status,
      selectedFile: undefined,
  ...
  onSuccess(uploadedImage) {
    ...
    this.resetToDefaultState('OK', uploadedImage);
  ...
  uploadImage() {
    ...
    deleteImage(imageURL) {
      ...
      actions.deleteImage(imageURL).then(   
        (message) => {this.onDeleteSuccess(message)},
        (error) => { this.onError(error)})
    }
    ...
    return (
      ...
      <div className='img-upload-container'>
        ...
        { (status === 'OK') &&
          <div className='img-preview-container'>
            <div className='img-preview'
                 style={{'backgroundImage': 'url(' + displayUploadedImage + ')'}}>
            </div>
          </div>
        }
<div className=''>
        ...
        { selectedFile && 
          <button className='btn btn-bwm ma1'
                  type='button'
